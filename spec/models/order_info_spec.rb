...
RSpec.describe OrderInfo, type: :model do
  describe '購入' do
    before do
      ...
      @order_info = FactoryBot.build(:order_info, user_id: user.id, country_id: country.id, plane_id: plane.id,
                                                  plane_go_id: plane_go.id, plane_back_id: plane_back.id, hotel_id: hotel.id, tour_id: tour.id)
    end
